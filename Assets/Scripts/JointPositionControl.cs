...
public class JointPositionControl : MonoBehaviour
{
    ...
    private KinectBodySource _bodySourceManager;
    private int _bodyIndex = 0;
    private Matrix4x4 _fromKinecttoUnity = Matrix4x4.zero;
    private readonly int[,] _bonechains = new int[5, 4] { { 0, 1, 20, 2 }, { 0, 12, 13, 14 }, { 0, 16, 17, 18 }, { 20, 4, 5, 6 }, { 20, 8, 9, 10 } };
    ...
    void Start()
    {
        ...
        if (_kinectBodySource != null)
            _bodySourceManager = _kinectBodySource.GetComponent<KinectBodySource>();
    ...
    {
        const float boneWidth = .05f;
        for (int chain = 0; chain < _bonechains.GetLength(0); chain++)
        {
            for (int bone = 1; bone < _bonechains.GetLength(1); bone++)
            {
                var boneLenght = (AvatarBones[_bonechains[chain, bone]].transform.position - AvatarBones[_bonechains[chain, bone - 1]].transform.position).magnitude;
                AvatarBones[_bonechains[chain, bone]].transform.GetChild(0).transform.position = AvatarBones[_bonechains[chain, bone]].transform.position - 
                    (AvatarBones[_bonechains[chain, bone]].transform.position -
                     AvatarBones[_bonechains[chain, bone - 1]].transform.position)/2;
                AvatarBones[_bonechains[chain, bone]].transform.GetChild(0).transform.localScale = new Vector3(boneWidth, boneLenght, boneWidth);
            }
        }
    }
    ...
    private void MoveBones(Body kinectBody)
    {
        ...
        foreach (var bone in AvatarBones)
        {
            ...
            if (kinectBody.IsTracked)
            {
                ...
                bone.transform.position = _kinectBodySource.transform.position + _kinectBodySource.transform.rotation * unityPosition;
            }
        }
    }
